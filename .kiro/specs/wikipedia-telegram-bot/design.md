...
- Timeout (>5 секунд): Сообщение пользователю "Не удалось получить статью, попробуйте позже" с inline-кнопкой "Ещё" для повторной попытки
- HTTP ошибки (4xx, 5xx): Логирование и сообщение пользователю с inline-кнопкой "Ещё"
...
При любой ошибке получения статьи (Wikipedia API timeout, HTTP ошибки, общие ошибки):
- Отправить локализованное сообщение об ошибке
- Добавить inline-кнопку "Ещё" для повторной попытки
- Пользователь может нажать кнопку для новой попытки получения статьи
